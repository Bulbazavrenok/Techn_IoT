...
class FileDatasource:
    ...
    def read(self) -> List[AggregatedData]:
        ...
        for i in range(self.rows_to_return):
            ...
            list_of_data.append(AggregatedData(Accelerometer(*next(self.accelerometer_list_reader)),
                                               Gps(*next(self.gps_list_reader)),
                                               Parking(int(list_of_parking[0]), list_of_parking[1:]),
                                               datetime.now(),
                                               agent_id=AGENT_ID)
                                )
        return list_of_data
